...
Genius Invokation TCG是一款回合制的卡牌游戏。每场决斗分为三个阶段：准备阶段 (Preparation) > 回合阶段 (Rounds) > 胜负判定阶段 (Victory Evaluation)。每个回合阶段将按照以下顺序进行：掷骰阶段 (Roll Phase) > 行动阶段 (Action Phase) > 结束阶段 (End Phase)。
...
行动可以分为两种类型：战斗行动 (Combat Action) 和快速行动 (Fast Action)

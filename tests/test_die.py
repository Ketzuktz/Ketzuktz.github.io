...
class TestDieState:
    ...
    def test_subtraction_1(self):
        ...
        result = die_state1 - die_state2
        assert result.Omni == 3
        assert result.Pyro == 2
        assert result.Hydro == 1
    ...
    def test_comparison(self):
        ...
        for en in die_element_names:
            ...
            assert not (die_state1 > die_state2)
            assert not (die_state1 < die_state2)
            assert die_state1 <= die_state2
...
class TestRollNDies:
    def test_roll_n_dies_raw(self, mocker):
    ...
    def test_roll_n_dies(self, mocker):
        def test_mock_roll_n_dies(die_count: int):
            ...
            assert result.__dict__ == DieState(**kwargs).__dict__

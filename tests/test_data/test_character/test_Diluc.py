...
def test_Diluc_elemental_burst() -> None:
    game: GameManager = prepare_game()

    game.take_operation(DEBUG_GetDie({DieTypeEnum.omni: 4}, PlayerID(1)))
    game.take_operation(DEBUG_GetEnergy(3, CharacterID(1), PlayerID(1)))

    game.take_operation(
        PlayerOpUseSkill(
            skill_id=SkillID("Dawn"),
            player_id=PlayerID(1),
            character_id=CharacterID(1),
        )
    )

    assert game.game_state.side2.characters[0].hp == 2
    assert game.game_state.side1.characters[0].energy == 0
